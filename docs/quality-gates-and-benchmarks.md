...
For the local reviewer slice, repeated-run evidence is now directly consumable for the focused-VOI preference-burden family. The reviewer-indexed burden surfaces now point at `out/artifacts/full_suite_curated_latest_20260411_focused_voi_proof/` together with the local seed companions `_seed20260421` and `_seed20260522`, rather than only at the older single-seed `thesis_eval_20260331_r2_focused_voi/` slice. Those local repeated-seed artifacts support `P14.17-P14.20` with `median_preference_query_count = 0.0`, `p90_preference_query_count <= 1.0`, `max_preference_query_count <= 1.0`, and `preference_certification_success_rate` reported jointly. This narrower repeated-seed evidence still does not override the remaining suite-level blockers in DCCS, refine-cost forecasting, and VOI.
...
The root reviewer index now also names `table.focused_voi.preference_burden_summary` and `table.focused_voi.preference_burden_by_cohort` as maintained source surfaces backed by `thesis_summary.*`, `thesis_summary_by_cohort.*`, and `evaluation_manifest.json` from that repeated-seed focused-VOI proof bundle and its seed companions. Those surfaces expose the maintained preference-burden fields `median_preference_query_count`, `p90_preference_query_count`, `max_preference_query_count`, and `preference_certification_success_rate`. For the current checked local slice, read them as the checked local closure surfaces for `P14.17-P14.20`; they remain family-specific and do not by themselves green the full suite.
