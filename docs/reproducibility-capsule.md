...
Applies To: deterministic benchmark workflows, checked evaluation bundles, and artifact provenance under `backend/out/*`
...
As of `2026-04-09`, there is no checked `backend/out/capsule` directory in the repo, so the latest reproducibility evidence lives in manifests, preflight artifacts, and evaluation bundles rather than in a pre-generated capsule export.
...
For evaluation-lane reproduction, use the same OD corpus, budgets, and strict-evidence settings recorded in `backend/out/thesis_campaigns/dominance_cluster5_cardiff_bath_corr12p5_r2/tranche_001/artifacts/dominance_cluster5_cardiff_bath_corr12p5_r2_t001/methods_appendix.md`.
...
The latest checked evaluation-bundle repo preflight is `backend/out/thesis_campaigns/dominance_cluster5_cardiff_bath_corr12p5_r2/tranche_001/artifacts/dominance_cluster5_cardiff_bath_corr12p5_r2_t001/repo_asset_preflight.json` at `2026-04-06T09:36:17Z`, also with `required_ok: true`.
...
### Evaluation-Lane Parameter Anchors

The current methods appendix for the newest checked evaluation campaign records:
...
- fixed comparator policy where evaluation runs use `local_service` or another declared baseline mode
- fixed route-graph asset or subset asset when the evaluation lane stages one explicitly
...
4. benchmark or evaluation command line
...
The checked focused-VOI bundle at `backend/out/artifacts/thesis_eval_20260331_r2_focused_voi/` now also carries additively backfilled `index.json` and `index.md` as bundle-level inspection entrypoints for artifact presence and export-status checks only. Treat those files as current inspection aids for this checked local bundle, not as proof that every older legacy-named evaluation bundle originally emitted them; older pre-refresh bundles may still legitimately lack those index files.
...
5. Same comparator policy and route-graph subset policy for evaluation lanes.
