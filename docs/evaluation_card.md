...
This page is the evaluator-facing reference for the current checked backend evaluation lanes, cohort labels, and checked artifacts.
...
The evaluator in this repository is not a single monolithic benchmark. It is a set of named lanes and evidence bundles that support different parts of the certification/evaluation pipeline:
...
| Lane label used in docs | Status in inspected evaluator source | Evidence basis | Evidence-safe wording |
| --- | --- | --- | --- |
...
| `perturbation / flip-radius` | present | `run_thesis_evaluation.py` role `perturbation_flip_radius`, plus checked full-suite root `out/headline_exports/current_checked/full_suite_curated_latest_20260411/` | explicit suite role; checked companion/full-suite sample-size and publishability surfaces now expose the perturbation sample-size and gate counters |
...
| Lane label used in docs | Status in checked artifacts | Evidence basis | Evidence-safe wording |
| --- | --- | --- | --- |
...
- the quality-gates page records current local artifacts and evaluation bundles
...
- `docs/reviewer_quickstart.md`
  - checked bundle references
  - source-surface regeneration workflow
- `docs/negative_results.md`
  - limitations and overclaim boundaries
...
- some evaluation bundles exist with report and summary outputs, especially focused-VOI and campaign snapshots cited elsewhere in the docs
...
- current evaluation source registers lane purpose, row-count target wiring, cohort/support-bin composition references, and any configured headline seed-repeat plan, but the checked bundles in this slice should not be read as proving a per-run lane-metadata artifact unless one is cited explicitly
...
- the repo now also carries `backend/scripts/run_full_latest_suite.py` as the publishability-facing regeneration entrypoint, and the latest checked full-suite assessment bundle (`full_suite_curated_latest_20260411`) now reports `publishable_on_current_evidence = true`, `adoption_claim_supported = true`, `sample_size_failure_count = 0`, `fairness_failure_count = 0`, and `hot_rerun_all_green = true`, with `publishability_blockers = []`
...
- the latest checked full-suite assessment bundle is publishable on the applied-evidence policy and adoption-supported for the measured benchmark framing: headline adoption checks are green and evaluation-size requirements are met, while the suite verdict still carries DCCS, refine-cost, and VOI diagnostics for stronger certification claims. The separate pair-benchmark hot-rerun bundle remains green on its own, and the repaired suite verdict now agrees that hot rerun is no longer a blocker
...
- headline lane seed-repeat plans should be distinguished from completed repeated-seed evidence: the current full-suite bundle has completed multi-seed rows for `broad_cold_proof`, `focused_refc_proof`, and `focused_voi_proof`, but other lanes may still be plan-level or single-seed
- repeated seed-summary, BCa-bootstrap CI, Holm-adjusted claim-summary, and report-table artifacts count as stronger evidence only where the repeated runs themselves are present. In the current checked full-suite summary, `headline_seed_failure_count = 0`, `headline_claim_rows_evaluated = 9`, `headline_claim_narrowing_count = 0`, and `headline_bootstrap_shortfall_count = 0`
- bootstrap-ready reviewer summaries and report-table exports improve claim-discipline reporting, but they do not by themselves satisfy unrelated calibration-ECE, witness-sparsity, or deployment-validation questions
- failure-atlas completeness is currently evidenced by `out/headline_exports/current_checked/full_suite_curated_latest_20260411/failure_atlas_lane_metadata.json` (`lane_status = present_complete`, `row_count = 82`, required kinds present), while deployment-grade evidence remains outside the current checked applied-evidence claim
...
2. `docs/negative_results.md` for limitations and claim boundaries
...
The evaluator is already rich enough to support external review, but not every lane or cohort is fully mature.
