...
The appendix anchors currently used by those theorem docs are the existing report sections rather than a separate proof appendix: `Appendix D` (`Core backend models`), `Appendix N` (`Detailed Backend Runtime Module Narratives`), `Appendix R` (`Detailed Test Evidence Ledger`), `Appendix X2` (`Core Route Endpoints`), `Appendix AE10` / `Appendix AE11` (support and elicitation limits), and `Appendix AH21` (`Certification Effort Depends On Gap And Threshold`). The theorem docs own the exact code/test/artifact/evaluator mapping; this report owns the narrative landing zones for those families.
