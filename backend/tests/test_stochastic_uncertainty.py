...
def test_stochastic_uncertainty_metadata_includes_scenario_profile_fields() -> None:
    route = _route(distance_m=42_000.0, duration_s=2_700.0)
    full = build_option(
        route,
        option_id="scenario_full",
        vehicle_type="rigid_hgv",
        scenario_mode=ScenarioMode.FULL_SHARING,
        cost_toggles=CostToggles(),
        stochastic=StochasticConfig(enabled=True, seed=50, sigma=0.1, samples=36),
        departure_time_utc=datetime(2026, 2, 12, 7, 45, tzinfo=UTC),
    )
    no_share = build_option(
        route,
        option_id="scenario_no",
        vehicle_type="rigid_hgv",
        scenario_mode=ScenarioMode.NO_SHARING,
        cost_toggles=CostToggles(),
        stochastic=StochasticConfig(enabled=True, seed=50, sigma=0.1, samples=36),
    ...
    full_meta = full.uncertainty_samples_meta or {}
    no_meta = no_share.uncertainty_samples_meta or {}
    assert full_meta.get("scenario_mode") == "full_sharing"
    assert no_meta.get("scenario_mode") == "no_sharing"
    assert "scenario_profile_version" in full_meta
    assert "scenario_sigma_multiplier" in full_meta
    assert float(no_meta.get("sigma", 0.0)) >= float(full_meta.get("sigma", 0.0))
